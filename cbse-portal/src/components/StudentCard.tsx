...
const GRADE_COLORS: Record<string, string> = {
  A1: "bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-900/30 dark:text-emerald-400 dark:border-emerald-800",
  A2: "bg-teal-50 text-teal-700 border-teal-200 dark:bg-teal-900/30 dark:text-teal-400 dark:border-teal-800",
  B1: "bg-sky-50 text-sky-700 border-sky-200 dark:bg-sky-900/30 dark:text-sky-400 dark:border-sky-800",
  B2: "bg-blue-50 text-blue-700 border-blue-200 dark:bg-blue-900/30 dark:text-blue-400 dark:border-blue-800",
  C1: "bg-violet-50 text-violet-700 border-violet-200 dark:bg-violet-900/30 dark:text-violet-400 dark:border-violet-800",
  C2: "bg-purple-50 text-purple-700 border-purple-200 dark:bg-purple-900/30 dark:text-purple-400 dark:border-purple-800",
  D: "bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-900/30 dark:text-amber-400 dark:border-amber-800",
  E: "bg-red-50 text-red-700 border-red-200 dark:bg-red-900/30 dark:text-red-400 dark:border-red-800",
};
function gradeColor(g: string) {
  return GRADE_COLORS[g] ?? "bg-slate-50 text-slate-600 border-slate-200 dark:bg-slate-800 dark:text-slate-400 dark:border-slate-700";
}
function barColor(total: number | null) {
  if (total === null) return "bg-slate-300 dark:bg-slate-700";
...
function SubjectRow({ subject }: { subject: SubjectMark }) {
  ...
  return (
    <div className="py-3 px-4 border-b border-slate-100 dark:border-slate-800 last:border-0 hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors">
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          <p className="text-sm font-medium text-slate-800 dark:text-slate-200 truncate">{subject.subjectName}</p>
        ...
        <div className="flex items-center gap-3 shrink-0">
          {subject.theory !== null && (
            <div className="flex flex-col items-end w-12">
              ...
              <p className="text-xs font-semibold text-slate-600 dark:text-slate-300 leading-tight mt-0.5">{subject.theory}</p>
              <div className="w-full h-1 bg-slate-100 dark:bg-slate-800 rounded-full mt-1 overflow-hidden">
                <div className="h-full bg-slate-400 dark:bg-slate-500 rounded-full" style={{ width: `${Math.min(theoryPct, 100)}%` }} />
              </div>
            </div>
          )}
          {subject.practical !== null && subject.practical > 0 && (
            <div className="flex flex-col items-end w-12">
              ...
              <p className="text-xs font-semibold text-slate-600 dark:text-slate-300 leading-tight mt-0.5">{subject.practical}</p>
              <div className="w-full h-1 bg-slate-100 dark:bg-slate-800 rounded-full mt-1 overflow-hidden">
                <div className="h-full bg-slate-400 dark:bg-slate-500 rounded-full" style={{ width: `${Math.min(pracPct, 100)}%` }} />
              </div>
            </div>
          )}
          <div className="flex flex-col items-end w-12">
            <p className="text-[10px] text-slate-400 uppercase tracking-wider">Total</p>
            <p className="text-sm font-bold text-slate-800 dark:text-slate-200 leading-tight">{subject.total ?? "—"}</p>
          </div>
...
export default function StudentCard({ student, scope, rank }: StudentCardProps) {
  ...
  return (
    <article
      className={`bg-white dark:bg-slate-900 rounded-2xl border shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden group ${
        expanded ? "ring-2 ring-blue-500 ring-offset-1 dark:ring-offset-slate-950" : "border-slate-200 dark:border-slate-800"
      }`}
    >
      <button
          ...
          <div className="flex flex-1 items-start gap-3 min-w-0">
            <div className="relative flex-shrink-0">
              <div className={`w-11 h-11 rounded-xl flex items-center justify-center text-lg font-bold ${
                student.sex === "F" ? "bg-pink-100 text-pink-700 dark:bg-pink-900/30 dark:text-pink-400" : "bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400"
              }`}>
            ...
            <div className="min-w-0">
              <h2 className="text-base font-bold text-slate-900 dark:text-slate-100 leading-tight group-hover:text-blue-700 dark:group-hover:text-blue-400 transition-colors truncate">
                {student.name}
              </h2>
              <div className="flex flex-wrap items-center gap-2 mt-1">
                <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 text-xs rounded-md font-mono font-medium">
                  {student.rollNumber}
                </span>
                {student.section && (
                  <span className="text-xs text-slate-400 dark:text-slate-500 bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded">{student.section}</span>
                )}
              </div>
          ...
          <div className="flex flex-col items-end gap-2 shrink-0">
            ...
            <div className="text-right">
              <p className="text-lg font-extrabold text-slate-800 dark:text-slate-100 leading-tight">
                {scopedTotal}
                <span className="text-xs font-normal text-slate-400 dark:text-slate-500"> / {scopedMax}</span>
              </p>
      ...
      <div id={`details-${student.rollNumber}`}
        ...
        <div className="border-t border-slate-100 dark:border-slate-800">
          {/* Info strip */}
          <div className="px-5 py-3 bg-slate-50 dark:bg-slate-900/50 grid grid-cols-2 sm:grid-cols-3 gap-3 text-xs">
            <div>
              <p className="text-slate-400 uppercase tracking-wider text-[10px] font-semibold">Father</p>
              <p className="text-slate-700 dark:text-slate-300 font-medium mt-0.5">{student.fatherName}</p>
            </div>
            <div>
              <p className="text-slate-400 uppercase tracking-wider text-[10px] font-semibold">Mother</p>
              <p className="text-slate-700 dark:text-slate-300 font-medium mt-0.5">{student.motherName}</p>
            </div>
            <div>
              <p className="text-slate-400 uppercase tracking-wider text-[10px] font-semibold">Session</p>
              <p className="text-slate-700 dark:text-slate-300 font-medium mt-0.5">{student.session}</p>
            </div>
            <div>
              <p className="text-slate-400 uppercase tracking-wider text-[10px] font-semibold">{scopeLabel} Total</p>
              <p className="text-slate-700 dark:text-slate-300 font-bold mt-0.5 text-sm">{scopedTotal} / {scopedMax}</p>
            </div>
            <div>
              <p className="text-slate-400 uppercase tracking-wider text-[10px] font-semibold">Declaration</p>
              <p className="text-slate-700 dark:text-slate-300 font-medium mt-0.5">{student.dateOfDeclaration}</p>
            </div>
            <div>
              <p className="text-slate-400 uppercase tracking-wider text-[10px] font-semibold">Adm. No.</p>
              <p className="text-slate-700 dark:text-slate-300 font-mono font-medium mt-0.5">{student.id}</p>
            </div>
          </div>
            ...
            <div className="px-5 pb-4">
              ...
              <div className="flex flex-wrap gap-2">
                {student.internalSubjects.map((s, i) => (
                  <span key={i} className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-xs">
